...
impl Config {
    ...
    pub fn freeze(self) {
        ...
        match self.clock32ksrc {
            Clock32KSrc::LSE => {
                with_safe_access(|| {
                    sys.r8_ck32k_config().modify(|_, w| w.rb_clk_xt32k_pon().set_bit());
                });
                ...
                //});
                with_safe_access(|| {
                    sys.r8_ck32k_config().modify(|_, w| w.rb_clk_osc32k_xt().set_bit());
                });
            ...
            Clock32KSrc::LSI => {
                with_safe_access(|| {
                    sys.r8_ck32k_config()
                        .modify(|_, w| w.rb_clk_osc32k_xt().clear_bit().rb_clk_int32k_pon().set_bit());
                });
            }
        }

        with_safe_access(|| unsafe {
            sys.r8_pll_config().modify(|r, w| w.bits(r.bits() & !(1 << 5)));
        });
        let hclk = match self.mux {
            ClockSrc::HSE(div) => {
                assert!(div != 1, "1 means close HCLK");
                if sys.r8_hfck_pwr_ctrl().read().rb_clk_xt32m_pon().bit_is_clear() {
                    // HSE power on
                    with_safe_access(|| sys.r8_hfck_pwr_ctrl().modify(|_, w| w.rb_clk_xt32m_pon().set_bit()));
                    unsafe {
                ...
                with_safe_access(|| {
                    sys.r16_clk_sys_cfg()
                        .write(|w| unsafe { w.rb_clk_sys_mod().bits(0b00).rb_clk_pll_div().bits(div & 0x1f) });
                    riscv::asm::nop();
                    riscv::asm::nop();
                    riscv::asm::nop();
                    riscv::asm::nop();
                ...
                riscv::asm::nop();
                with_safe_access(|| {
                    sys.r8_adc_cfg().write(|w| unsafe { w.bits(0x51) });
                });
                Hertz::from_raw(HSE_FREQUENCY.to_Hz() / (div as u32))
            }
            ClockSrc::PLL(div) => {
                assert!(div != 1, "1 means close HCLK");
                if sys.r8_hfck_pwr_ctrl().read().rb_clk_pll_pon().bit_is_clear() {
                    // HSE power on
                    with_safe_access(|| sys.r8_hfck_pwr_ctrl().modify(|_, w| w.rb_clk_pll_pon().set_bit()));
                    riscv::asm::delay(4000);
                }
                with_safe_access(|| {
                    sys.r16_clk_sys_cfg()
                        .write(|w| unsafe { w.rb_clk_sys_mod().bits(0b01).rb_clk_pll_div().bits(div & 0x1f) });
                    riscv::asm::nop();
                    riscv::asm::nop();
                    riscv::asm::nop();
                    riscv::asm::nop();
                ...
                if div == 6 {
                    ...
                    with_safe_access(|| unsafe {
                        sys.r8_adc_cfg().modify(|_, w| w.bits(0x52));
                    });
                }
            ...
            _ => {
                // directly from CK32
                with_safe_access(|| unsafe {
                    sys.r16_clk_sys_cfg().modify(|r, w| w.bits(r.bits() | 0xC0));
                });
                Hertz::from_raw(32_768)
            }
        };
        with_safe_access(|| unsafe {
            sys.r8_pll_config().modify(|r, w| w.bits(r.bits() | (1 << 7)));
        });
