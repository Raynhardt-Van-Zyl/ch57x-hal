...
pub(crate) mod sealed {
    ...
    pub trait Instance {
        ...
        fn regs() -> &'static crate::pac::sys::RegisterBlock;
    }
    ...
    T: Instance,
{
    pub fn new(adc: impl Peripheral<P = T> + 'd, config: Config) -> Self {
        ...
        let rb = T::regs();
        rb.r8_adc_cfg().modify(|_, w| {
            unsafe { w.rb_adc_power_on()
                .set_bit()
                .rb_adc_diff_en()
    ...
    pub fn enable_temperature(&self) -> Temperature {
        ...
        rb.r8_tem_sensor().modify(|_, w| w.rb_tem_sen_pwr_on().set_bit());
    ...
    fn convert(&mut self) -> u16 {
        ...
        // start adc convert
        rb.r8_adc_convert().modify(|_, w| w.rb_adc_start().set_bit());
        // wait for convert
        while rb.r8_adc_convert().read().rb_adc_start().bit_is_set() {}

        rb.r16_adc_data().read().rb_adc_data().bits()
    }
    ...
    pub fn read(&mut self, pin: &mut impl AdcPin<T>) -> u16 {
        ...
        // Select channel
        rb.r8_adc_channel().modify(|_, w| unsafe { w.rb_adc_ch_inx().bits(channel) });
    ...
    pub fn read_as_millivolts(&mut self, pin: &mut impl AdcPin<T>) -> i32 {
        ...
        let vref = 1050;
        // Ref: DS manual
        match rb.r8_adc_cfg().read().rb_adc_pga_gain().bits() {
            // -12dB, 1/4
...
impl<'d, T: Instance> Drop for Adc<'d, T> {
    fn drop(&mut self) {
        ...
        rb.r8_adc_cfg().modify(|_, w| w.rb_adc_power_on().clear_bit());
    }
}
