...
impl SystickDriver {
    fn init(&'static self) {
        ...
        rb.r32_stk_cmphr().write(|w| unsafe { w.bits((u64::MAX - 1) as u32) });
        critical_section::with(|_| {
            rb.r32_stk_cntfg().write(|w| w.cntif().bit(false)); // clear
                                                     // Configration: Upcount, No reload, HCLK/8 as clock source
            rb.r32_stk_ctlr().modify(|_, w| {
                w.ste()
                    .set_bit()
                    .stclk()
                    .clear_bit()
    ...
    fn on_interrupt(&self) {
        let rb = unsafe { &*pac::Systick::PTR };
        rb.r32_stk_ctlr().modify(|_, w| w.stie().clear_bit()); // disable interrupt
        rb.r32_stk_cntfg().write(|w| w.cntif().bit(false)); // clear IF
...
impl Driver for SystickDriver {
    fn now(&self) -> u64 {
        let rb = unsafe { &*pac::Systick::PTR };
        (rb.r32_stk_cnth().read().bits() as u64) / (self.period.load(Ordering::Relaxed) as u64)
    }
    ...
    fn set_alarm(&self, alarm: AlarmHandle, timestamp: u64) -> bool {
        critical_section::with(|cs| {
            ...
            if timestamp <= t {
                ...
                rb.r32_stk_ctlr().modify(|_, w| w.stie().clear_bit());
            ...
            rb.r32_stk_cmphr().write(|w| unsafe { w.bits(safe_timestamp as u32) });
            rb.r32_stk_ctlr().modify(|_, w| w.stie().set_bit());
