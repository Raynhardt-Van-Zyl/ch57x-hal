...
pub mod dma;
pub mod gpio;
// pub mod i2c; // I2C not available in CH57x
// pub mod lcd;
#[cfg(feature = "ble")]
pub mod ble;
pub mod delay;
...
pub fn init(config: Config) -> Peripherals {
    ...
    if config.enable_dcdc {
        with_safe_access(|| {
            sys.r8_aux_power_adj().modify(|_, w| unsafe { w.rb_ulpldo_adj().bits(1) });
            sys.r16_power_plan().modify(|_, w| w.rb_pwr_dcdc_pre().set_bit());
        });
        delay_us(10);
        with_safe_access(|| {
            sys.r16_power_plan().modify(|_, w| w.rb_pwr_dcdc_en().set_bit());
        });
    } else {
        with_safe_access(|| {
            sys.r8_aux_power_adj().modify(|_, w| unsafe { w.rb_ulpldo_adj().bits(0) });
            sys.r16_power_plan()
                .modify(|_, w| w.rb_pwr_dcdc_pre().clear_bit().rb_pwr_dcdc_en().clear_bit());
        });
    }
    ...
    if config.low_power {
        unsafe {
            let rb = &*pac::Sys::PTR;
            // PA
            rb.r32_pa_pd_drv().write(|w| w.bits(0));
            rb.r32_pa_pu().write(|w| w.bits(0xffff));
            rb.r32_pa_dir().write(|w| w.bits(0));
            // PB
            rb.r32_pb_pd_drv().write(|w| w.bits(0));
            rb.r32_pb_pu().write(|w| w.bits(0xffff));
            rb.r32_pb_dir().write(|w| w.bits(0));
        }
    }
...
pub unsafe fn reset() -> ! {
    ...
    pfic.r32_pfic_cfgr().write(|w| unsafe { w.keycode().bits(KEY3).sysreset().set_bit() });
    loop {}
}
...
pub unsafe fn soft_reset() -> ! {
    ...
    with_safe_access(|| {
        rb.r8_rst_wdog_ctrl().modify(|_, w| w.rb_software_reset().set_bit());
    });
    loop {}
}
