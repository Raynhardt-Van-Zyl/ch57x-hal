...
impl Rtc {
    ...
    pub fn timestamp_since_epoch(&self) -> u32 {
        let rtc = unsafe { &*pac::Sys::PTR };

        let day = rtc.r32_rtc_cnt_day().read().bits() & 0x3fff;
        let mut sec = rtc.r16_rtc_cnt_2s().read().bits() * 2;
        if rtc.r16_rtc_cnt_32k().read().bits() >= 0x8000 {
            sec += 1;
        }
    ...
    pub fn counter_2s(&self) -> u16 {
        let rtc = unsafe { &*pac::Sys::PTR };
        rtc.r16_rtc_cnt_2s().read().bits()
    }

    pub fn counter_tick(&self) -> u32 {
        let rtc = unsafe { &*pac::Sys::PTR };
        (rtc.r16_rtc_cnt_2s().read().bits() as u32) << 16 | (rtc.r16_rtc_cnt_32k().read().bits() as u32)
    }

    pub fn counter_day(&self) -> u16 {
        let rtc = unsafe { &*pac::Sys::PTR };
        (rtc.r32_rtc_cnt_day().read().bits() & 0x3fff) as u16
    }

    pub fn enable_timing(&mut self, mode: TimingMode) {
        let rtc = unsafe { &*pac::Sys::PTR };
        rtc.r8_rtc_flag_ctrl().modify(|_, w| w.rb_rtc_tmr_clr().set_bit()); // clear flag
        with_safe_access(|| unsafe {
            rtc.r8_rtc_mode_ctrl()
                .modify(|_, w| w.rb_rtc_tmr_mode().bits(mode as u8).rb_rtc_tmr_en().set_bit());
        });
    }
    ...
    pub fn disable_timing(&mut self) {
        let rtc = unsafe { &*pac::Sys::PTR };
        with_safe_access(|| {
            rtc.r8_rtc_mode_ctrl().modify(|_, w| w.rb_rtc_tmr_en().clear_bit());
        });
    }
    ...
    pub fn set_datatime(&mut self, t: DateTime) {
        ...
        with_safe_access(|| unsafe {
            rtc.r32_rtc_trig().write(|w| w.bits(days as u32));
            rtc.r8_rtc_mode_ctrl().modify(|_, w| w.rb_rtc_load_hi().set_bit());
        });
        // load_hi clear when loadded
        while rtc.r8_rtc_mode_ctrl().read().rb_rtc_load_hi().bit_is_set() {}
        let t = (sec2 as u32) << 16 | (t32k as u32);
        with_safe_access(|| unsafe {
            rtc.r32_rtc_trig().write(|w| w.bits(t));
            rtc.r8_rtc_mode_ctrl().modify(|_, w| w.rb_rtc_load_lo().set_bit());
        });
    }
    ...
    pub fn now(&self) -> DateTime {
        ...
        let mut days = (rtc.r32_rtc_cnt_day().read().bits() & 0x3fff) as u16;
        // to u32, avoid overflow
        let mut sec = (rtc.r16_rtc_cnt_2s().read().bits() as u32) * 2;
        let cnt_32k = rtc.r16_rtc_cnt_32k().read().bits();
        if cnt_32k >= 0x8000 {
            sec += 1;
        }
