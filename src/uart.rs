...
impl<'d, T: BasicInstance> UartTx<'d, T> {
    ...
    */
    ...
    pub fn blocking_write(&mut self, buffer: &[u8]) -> Result<(), Error> {
        ...
        for &c in buffer {
            // wait
            while rb.r8_uart0_fcr().read().bits() >= UART_FIFO_SIZE {}
            rb.r8_uart0_thr().write(|w| unsafe { w.bits(c) });
        }
        Ok(())
    }
    ...
    pub fn blocking_flush(&mut self) -> Result<(), Error> {
        ...
        while rb.r8_uart0_fcr().read().bits() != 0 {}
        Ok(())
    }
}
...
impl<'d, T: BasicInstance> UartRx<'d, T> {
    ...
    fn check_rx_flags(&self) -> Result<bool, Error> {
        let rb = T::regs();
        let lsr = rb.r8_uart0_lsr().read();
        if lsr.rb_lsr_err_rx_fifo().bit() {
            if lsr.rb_lsr_frame_err().bit() {
                return Err(Error::Framing);
            } else if lsr.rb_lsr_par_err().bit() {
                return Err(Error::Parity);
            } else if lsr.rb_lsr_over_err().bit() {
                return Err(Error::Overrun);
            }
        }
        Ok(lsr.rb_lsr_data_rdy().bit())
    }
    ...
    pub fn blocking_read(&mut self, buffer: &mut [u8]) -> Result<(), Error> {
        ...
        for b in buffer {
            while !self.check_rx_flags()? {}
            if rb.r8_uart0_fcr().read().bits() > 0 {
                *b = rb.r8_uart0_rbr().read().bits() as u8;
            }
        }
        Ok(())
    }
    ...
    pub fn nb_read(&mut self) -> Result<u8, nb::Error<Error>> {
        ...
        if self.check_rx_flags()? {
            Ok(rb.r8_uart0_rbr().read().bits() as u8)
        } else {
...
fn configure(
    ...
    rb.r8_uart0_fcr().write(|w| {
        unsafe { w.rb_fcr_rx_fifo_clr()
            .set_bit()
            .rb_fcr_tx_fifo_clr()
            .set_bit()
            .rb_fcr_fifo_en() // enable 8 byte FIFO
            .set_bit()
            .rb_fcr_fifo_trig()
            .bits(0b00) } // 1 bytes to send
    });
    rb.r8_uart0_lcr().write(|w| unsafe { w.rb_lcr_word_sz().bits(config.data_bits as u8) });
    match config.stop_bits {
        StopBits::STOP1 => { rb.r8_uart0_lcr().modify(|_, w| w.rb_lcr_stop_bit().clear_bit()); },
        StopBits::STOP2 => { rb.r8_uart0_lcr().modify(|_, w| w.rb_lcr_stop_bit().set_bit()); },
    }
    match config.parity {
        Parity::ParityNone => { rb.r8_uart0_lcr().modify(|_, w| w.rb_lcr_par_en().clear_bit()); },
          _ => { rb
            .r8_uart0_lcr()
            .modify(|_, w| unsafe { w.rb_lcr_par_en().set_bit().rb_lcr_par_mod().bits(config.parity as u8) }); },
    }
    ...
    rb.r8_uart0_div().write(|w| unsafe { w.bits(div) });
    rb.r16_uart0_dl().write(|w| unsafe { w.bits(dl) });

    // enable TX
    if enable_tx {
        rb.r8_uart0_ier().modify(|_, w| w.rb_ier_txd_en().bit(true));
    }
    ...
    ($inst:ident, $irq:ident, $remap_field:ident) => {
        impl sealed::BasicInstance for crate::peripherals::$inst {
            ...
            fn regs() -> &'static crate::pac::uart0::RegisterBlock {
                unsafe { &*(crate::pac::$inst::PTR as *const crate::pac::uart0::RegisterBlock) }
            }

            /// Remap offset in R16_PIN_ALTERNATE
            fn set_remap() {
                let gpioctl = unsafe { &*pac::Sys::PTR };
                gpioctl.r16_pin_alternate().modify(|_, w| w.$remap_field().set_bit());
            }
        }
...
impl_uart!(Uart0, UART0, rb_pin_uart0); // FIXME: UartWithModem, a FullInstance
impl_uart!(Uart1, UART1, rb_pin_uart0);
impl_uart!(Uart2, UART2, rb_pin_uart0);
impl_uart!(Uart3, UART3, rb_pin_uart0);
