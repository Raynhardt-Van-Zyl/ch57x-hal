...
impl<'d, T: Pin> Flex<'d, T> {
    ...
    pub fn is_low(&self) -> bool {
        ...
        let bits = if self.pin._port() == 0 {
            rb.r32_pa_pin().read().bits()
        } else {
            rb.r32_pb_pin().read().bits()
        };
        bits & (1 << self.pin.pin()) == 0
    }
    ...
    pub fn is_set_low(&self) -> bool {
        ...
        let bits = if self.pin._port() == 0 {
            rb.r32_pa_out().read().bits()
        } else {
            rb.r32_pb_out__r8_slv_rd_data().read().bits()
        };
        bits & mask == 0
    }
...
pub(crate) unsafe fn init() {
    ...
    interrupt::GPIO_A::disable();
    interrupt::GPIO_A::set_priority(interrupt::Priority::P3);
    interrupt::GPIO_A::enable();

    interrupt::GPIO_B::disable();
    interrupt::GPIO_B::set_priority(interrupt::Priority::P3);
    interrupt::GPIO_B::enable();
}
...
fn irq_handler<const N: usize>(port: u8, wakers: &[AtomicWaker; N]) {
    ...
    let int_if = if port == 0 {
        gpioctl.r16_pa_int_if().read().bits()
    } else {
        gpioctl.r16_pb_int_if().read().bits()
    };
    for pin in 0..16 {
        if int_if & (1 << pin) == 0 {
            continue;
        ...
        if port == 0 {
            unsafe {
                ...
                gpioctl.r16_pa_int_if().write(|w| w.bits(1 << pin));
                gpioctl.r16_pa_int_en().modify(|r, w| w.bits(r.bits() & !(1 << pin)));
            }
            wakers[pin as usize].wake();
        } else if port == 1 {
            unsafe {
                gpioctl.r16_pb_int_if().write(|w| w.bits(1 << pin));
                gpioctl.r16_pb_int_en().modify(|r, w| w.bits(r.bits() & !(1 << pin)));
            }
            let pb8_9_remapped = gpioctl.r16_pin_alternate().read().bits() & (1 << 0) != 0;
...
pub(crate) mod sealed {
    ...
    pub trait Pin {
        ...
        #[inline]
        fn block(&self) -> &'static pac::sys::RegisterBlock {
            unsafe { &*pac::Sys::PTR }
        }
        ...
        fn set_high(&self) {
            ...
            let n = self._pin();
            if self._port() == 0 {
                rb.r32_pa_out().modify(|r, w| unsafe { w.bits(r.bits() | (1 << n)) });
            } else {
                rb.r32_pb_out__r8_slv_rd_data()
        ...
        fn set_low(&self) {
            ...
            let n = self._pin();
            if self._port() == 0 {
                rb.r32_pa_clr().write(|w| unsafe { w.bits(1 << n) });
            } else {
                rb.r32_pb_clr().write(|w| unsafe { w.bits(1 << n) });
            }
        }
        ...
        fn set_as_analog(&self) {
            ...
            unsafe {
                if self._port() == 0 {
                    rb.r32_pa_pd_drv().modify(|r, w| w.bits(r.bits() & !(1 << pin)));
                    rb.r32_pa_pu().modify(|r, w| w.bits(r.bits() & !(1 << pin)));
                    rb.r32_pa_dir().modify(|r, w| w.bits(r.bits() & !(1 << pin)));
                } else {
                    rb.r32_pb_pd_drv().modify(|r, w| w.bits(r.bits() & !(1 << pin)));
                    rb.r32_pb_pu().modify(|r, w| w.bits(r.bits() & !(1 << pin)));
                    rb.r32_pb_dir().modify(|r, w| w.bits(r.bits() & !(1 << pin)));
                }
            }
        }
        ...
        fn disable_interrupt(&mut self) {
            critical_section::with(|_| {
                ...
                match self._port() {
                    0 => unsafe {
                        gpioctl.r16_pa_int_en().modify(|r, w| w.bits(r.bits() & !(1 << n)));
                    },
                    1 if n >= 22 => unsafe {
                        // map PB[23:22] to PB[9:8]
                        gpioctl.r16_pb_int_en().modify(|r, w| w.bits(r.bits() & !(1 << (n - 14))));
                    },
                    1 => unsafe {
                        gpioctl.r16_pb_int_en().modify(|r, w| w.bits(r.bits() & !(1 << n)));
                    },
                    _ => unreachable!(),
                }
            })
        ...
        fn is_interrupt_enabled(&self) -> bool {
            ...
            let n = self._pin();
            match self._port() {
                0 => gpioctl.r16_pa_int_en().read().bits() & (1 << n) != 0,
                1 if n >= 22 => {
                    // map PB[23:22] to PB[9:8]
                    gpioctl.r16_pb_int_en().read().bits() & (1 << (n - 14)) != 0
                }
                1 => gpioctl.r16_pb_int_en().read().bits() & (1 << n) != 0,
                _ => unreachable!(),
            }
        }
                ...
                match self._port() {
                    0 => unsafe {
                        if matches!(trigger, LowLevel | HighLevel) {
                            gpioctl.r16_pa_int_mode().modify(|r, w| w.bits(r.bits() & !(1 << n)));
                        } else {
                            gpioctl.r16_pa_int_mode().modify(|r, w| w.bits(r.bits() | (1 << n)));
                        }
                    },
                    1 => unsafe {
                        if n >= 22 {
                            n -= 14;
                            gpioctl.r16_pin_alternate().modify(|r, w| w.bits(r.bits() | (1 << 0)));
                        }

                        if matches!(trigger, LowLevel | HighLevel) {
                            gpioctl.r16_pb_int_mode().modify(|r, w| w.bits(r.bits() & !(1 << n)));
                        } else {
                            gpioctl.r16_pb_int_mode().modify(|r, w| w.bits(r.bits() | (1 << n)));
                        }
                    },
        ...
        fn enable_interrupt(&mut self) {
            critical_section::with(|_| {
                ...
                match self._port() {
                    0 => unsafe {
                        gpioctl.r16_pa_int_if().write(|w| w.bits(1 << n));
                        gpioctl.r16_pa_int_en().modify(|r, w| w.bits(r.bits() | (1 << n)));
                    },
                    1 => unsafe {
                        if n >= 22 {
                        ...
                        gpioctl.r16_pb_int_if().write(|w| w.bits(1 << n));
                        gpioctl.r16_pb_int_en().modify(|r, w| w.bits(r.bits() | (1 << n)));
                    },
                    _ => unreachable!(),
                }
        ...
        fn clear_interrupt(&mut self) {
            ...
            match self._port() {
                ...
                1 if n >= 22 => unsafe {
                    // remap to PB[9:8]
                    gpioctl.r16_pb_int_if().modify(|r, w| w.bits(r.bits() | (1 << (n - 14))));
                },
                1 => unsafe {
                    gpioctl.r16_pb_int_if().write(|w| w.bits(1 << n));
                },
