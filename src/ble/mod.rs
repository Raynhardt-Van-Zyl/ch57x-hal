...
fn configure_pa(pa_config: PaConfig) {
    ...
    unsafe {
        ...
        PA_CONFIG.txEnableGPIO = tx_pin.block().r32_pa_out().as_ptr() as u32;
        PA_CONFIG.txDisableGPIO = tx_pin.block().r32_pa_clr().as_ptr() as u32;

        PA_CONFIG.rx_pin = rx_pin._pin() as u32;
        PA_CONFIG.rxEnableGPIO = rx_pin.block().r32_pa_out().as_ptr() as u32;
        PA_CONFIG.rxDisableGPIO = rx_pin.block().r32_pa_clr().as_ptr() as u32;
...
pub unsafe extern "C" fn srand() -> u32 {
    ...
    systick.r32_stk_cnth().read().bits() as u32
}
...
pub unsafe extern "C" fn temperature_raw() -> u16 {
    ...
    regs[0] = sys.r8_tkey_cfg().read().bits();
    regs[1] = rb.r8_tem_sensor().read().bits();
    regs[2] = rb.r8_adc_channel().read().bits();
    regs[3] = rb.r8_adc_cfg().read().bits();
    ...
    // restore regs
    sys.r8_tkey_cfg().write(|w| w.bits(regs[0]));
    rb.r8_tem_sensor().write(|w| w.bits(regs[1]));
    rb.r8_adc_channel().write(|w| w.bits(regs[2]));
    rb.r8_adc_cfg().write(|w| w.bits(regs[3]));
